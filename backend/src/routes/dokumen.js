...
const {
  ...
  PenjualanOnline, PenjualanOnlineItem, SuratJalanOnline, InvoiceOnline, ReturOnline,
  DocumentCounter,
  sequelize,
} = require('../models');
...
function normalizeOnlinePenjualan(data) {
  return {
    ...data,
    nama_penerima: data.nama_pelanggan,
    nama_npwp: data.nama_pelanggan,
    no_po: data.id_pesanan,
    pengirim_detail: data.alamat_detail,
    pengirim_kode_pos: data.kode_pos,
    pengirimProvinsi: data.provinsi,
    pengirimKabupaten: data.kabupaten,
    pengirimKecamatan: data.kecamatan,
    pengirimKelurahan: data.kelurahan,
    status: data.status === 'SELESAI' ? 'COMPLETED' : data.status,
  };
}
...
router.get('/surat-jalan-online/:id/print', authenticatePrint, async (req, res) => {
  try {
    ...
    if (!doc) return res.status(404).json({ message: 'Surat Jalan Online tidak ditemukan' });
...
router.get('/invoice-online/:id/print', authenticatePrint, async (req, res) => {
  try {
