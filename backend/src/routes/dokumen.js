...
const onlineDocHtml = (title, nomor, tanggal, penjualan) => {
  const p = penjualan || {};
  ...
  <div class="grid"><div class="box"><b>Pelanggan</b><br>${esc(p.nama_pelanggan)}<br>${esc(p.no_hp)}<br><span class="muted">${esc(alamat)}</span></div><div class="box"><b>Pesanan</b><br>ID Pesanan: ${esc(p.id_pesanan)}<br>Channel: ${esc(p.channel)}<br>Faktur: ${esc(p.faktur)}</div></div>
