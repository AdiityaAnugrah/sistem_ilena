...
export default function EmailDokumenModal({ open, onClose, tipe, docId, nomor, defaultEmail = '' }: Props) {
  ...
  const handleSend = async () => {
    ...
    setErrorMsg('');
    try {
      await api.post(getEmailPath(tipe, docId), { email: email.trim() });
      setStatus('success');
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      <div style={{ width: '100%', maxWidth: 400, background: '#fff', borderRadius: 18, boxShadow: '0 20px 60px rgba(15,23,42,0.22)', overflow: 'hidden' }}>
        {/* Header */}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: !email.trim() || status === 'loading' ? '#94a3b8' : '#FA2F2F',
